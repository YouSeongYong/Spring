...
<body>
	...
	<div id="wrapper">
		...
		</c:choose> <!-- 로그인을 안한 경우 --> <!-- 로그인을 한경우 --> <!-- 모든 계정 --> <!-- a : 로그아웃/ 개인정보수정 -->


		<!-- 관리자 계정 --> <!-- a: 전체 회원정보 --> <!-- 로그인 후 Logout.jsp로 이동할 수 있는'로그아웃'링크와 '개인정보수정'링크를 출력하시오. -->
		</nav> </header>
		...
		<div class="inner">
			<header class="major"> <!-- 로그인을 안한경우 --> <c:choose>
				<c:when test="${empty loginM}">
			...
			</c:choose> <!-- 로그인을 한경우 --> <!-- h1 : ~(이메일)님 환영합니다 --> <!-- 로그인 후 로그인 한 사용자의 세션아이디로 바꾸시오.
			...
			<form action="/message/mail/sendMail" method="post">
				...
				<div class="field half">
					<label for="email">Email</label> <input type="text"
						name="receiveEmail" id="email" placeholder="받는 사람 이메일" />
				</div>
	...
	$(".page-link").on("click",function(e){
		e.preventDefault(); //a태그 기능 막기 ( 이동막기)
